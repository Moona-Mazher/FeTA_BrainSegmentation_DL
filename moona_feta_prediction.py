...
@author: Moona
"""
######################################### prediction method for feta challenge##########################
#%% cornal prediciton
import torch
...
for i in patients:
    ...
    for file in range(0,img_data.shape[0]):
        ...
        with torch.no_grad():
            ...
            tta_modela=model_A
            tta_modela.eval()
            tta_modela.to(device)
            outputa = tta_modela(img_a.to(device))
            ...
            tta_modelc=model_C
            tta_modelc.eval()
            tta_modelc.to(device)
            outputc = tta_modelc(img_c.to(device))
            ...
            tta_models=model_S
            tta_models.eval()
            tta_models.to(device)
            outputs = tta_models(img_s.to(device))
